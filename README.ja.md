...
**1 — 対象プロジェクトの Codespace を開く。** Copilot が MCP を自動起動し、内蔵 bridge も一緒に立ちます。まっさらな Codespace の初回だけ1回ビルド(約30〜60秒、以降キャッシュ)。

**2 — 手元のマシンでプライベートトンネルを張る**(開いたまま):
...
**3 — Figma プラグイン**(「Yasuda Figma MCP」)を実行 → トークンを貼る → **Connect**(ステータスが **Ready** に)。

- Codespace のターミナルでトークンを表示: `echo "$BRIDGE_TOKEN"`。

**4 — Copilot**(agent mode)で利用。例: *「今の Figma の選択をスクショして」*。9 個の `yfigma_*` ツールが使えます。

プロジェクト毎にやるのは**トンネル + プラグイン Connect だけ**。ツール(ユーザー設定)・トークン(ユーザーシークレット)・bridge(内蔵)は最初から揃っています。
