...
| `update-selection` | 選択ノードを `root` で更新（spec と選択の**型一致が必要**） | frame=自身のprops / instance=props / text。**子の差分反映は未対応（E-2）** |
...
| `target` `update-selection` の子の差分反映（冪等） | ⏳ E-2 |
...
**次の一歩（E-2）：** `update-selection` の子を index＋型でマッチし、更新/追加/余剰削除で
冪等に差分反映する。
